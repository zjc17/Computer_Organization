...
module Idecode32(read_data_1,read_data_2,Instruction,read_data,ALU_result,
    ...
    input        Jal;                               //  来自控制单元，说明是JAL指令, 访问特殊寄存器 
    input        RegWrite;                  // 来自控制单元
    input        MemtoReg;              // 来自控制单元
    input        RegDst;                    //  来自控制单元
    output[31:0] Sign_extend;               // 译码单元输出的扩展后的32位立即数
    input		 clock,reset;                // 时钟和复位(上升沿)
    input[31:0]  opcplus4;                 // 来自取指单元，JAL中用($PC+4)
...
//    reg  ZeroExt;
    ...
    //嵌套
    assign Sign_extend[31:0] = (opcode==6'hc||opcode==6'hd)?{16'd0,Instruction_immediate_value}:(sign?{16'd65535,Instruction_immediate_value}:{16'd0,Instruction_immediate_value});
    
//    always @* begin
//        if(opcode==6'hc||opcode==6'hd)
//            ZeroExt = 1   
//    end
