...
module memorio(caddress,address,memread,memwrite,ioread,iowrite,mread_data,ioread_data,wdata,rdata,write_data,LEDCtrl,SwitchCtrl);
    ...
    assign  rdata = memread&&~iorw?mread_data:(~caddress[1:1]?{16'b0,ioread_data[15:0]}:{8'b0,ioread_data[7:0],rdata[15:0]});
    assign  iorw = (iowrite||ioread);
	
	//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	assign	LEDCtrl = (caddress==32'hFFFFFC60||caddress==32'hFFFFFC62);
	assign	SwitchCtrl = (caddress==32'hFFFFFC70||caddress==32'hFFFFFC72);
							
    always @* begin
        if((memwrite==1)||(iowrite==1)) begin
            write_data = memwrite&&~iorw?wdata:(~caddress[1:1]?{16'b0,wdata[15:0]}:{8'b0,wdata[7:0],rdata[15:0]});
        end else begin
